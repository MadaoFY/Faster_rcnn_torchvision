...
def non_max_suppression_fasterrcnn(prediction,
                        conf_thres=0.25,
                        iou_thres=0.45,
                        agnostic=False,
                        max_det=300):
...
class Colors:
    ...
    def __init__(self, catid_labels):
        base_hexs = ('FF3838', 'FF9D97', 'FF701F', 'FFB21D', 'CFD231', '48F90A', '92CC17', '3DDB86', '1A9334', '00D4BB',
                     '2C99A8', '00C2FF', '344593', '6473FF', '0018EC', '8438FF', '520085', 'CB38FF', 'FF95C8', 'FF37C7')
        n = len(catid_labels) / len(base_hexs)
        if n > 1:
            n = int(n) + 1
            base_hexs *= n

        self.labels_catid = tuple(catid_labels.keys())
        self.hex = base_hexs[:len(self.labels_catid)]
        self.catid_and_hex = {k: v for k, v in zip(self.labels_catid, self.hex)}

    def get_id_and_colors(self):
        id_and_colors = {k: self.hex2rgb(f'#{v}') for k, v in self.catid_and_hex.items()}
